...
export default function TableManagement() {
    ...
    const fetData = async () => {
      ...
      try {
        const res = await fetch(
          ServerUrl + "/api/reservations/locations/" + _id,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
  ...
  const updateTable = ({ number_of_seats, name, table_id }) => {
    const fetData = async () => {
      setLoading(true);
      try {
        const res = await fetch(
          ServerUrl + "/api/reservations/tables/" + table_id,
          {
            method: "PATCH",
        ...
        setTables((currentTables) => [
          ...currentTables.map((table) =>
            table._id === table_id ? { ...table, number_of_seats, name } : table
          ),
        ]);
      } catch (error) {
        setLoading(false);
  ...
  const updateLocation = ({ locationInRestaurant, location_id }) => {
    const fetData = async () => {
      setLoading(true);
      try {
        const res = await fetch(
          ServerUrl + "/api/reservations/locations/" + location_id,
          {
            method: "PATCH",
            headers: {
              "Content-Type": "application/json",
    ...
    <div className="w-full min-h-screen bg-[#f9fafb]">
      ...
      {loadingFirstOne ? (
        <div className="w-full h-full absolute top-0 left-0 flex items-center justify-center">
          <FadeLoader color={"#11cdef"} loading={loadingFirstOne} />
        </div>
      ) : (
        <div className="px-3 lg:px-5 py-4 lg:py-6 rounded-md min-h-fit w-full">
          <DndContext
            sensors={sensors}
    ...
    const addLocation = async () => {
      try {
        ...
        const refreshData = await fetch(
          ServerUrl + "/api/reservations/locations",
          {
            method: "GET",
          }
        );
  ...
  function addNewTable(location_id) {
    const addTable = async () => {
      try {
        ...
        const refreshData = await fetch(
          ServerUrl + "/api/reservations/tables",
          {
            method: "GET",
          }
        );
